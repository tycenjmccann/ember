...
export function cognitoConfig(): CognitoConfig | null {
  ...
  return { region, userPoolId, clientId, domain: domain.replace(/\/$/, "") };
}
 ...
 * signature, issuer, and audience (the app client id). Returns null on any
 * failure — callers treat that as unauthenticated.
 */
