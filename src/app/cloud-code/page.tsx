...
import { MarkdownRenderer } from "@/components/cloud-code/MarkdownRenderer";
import { CliBadge, CliMark, CLI_BRAND } from "@/components/cloud-code/CliBrand";
import VoiceButton from "@/components/cloud-code/VoiceButton";
...
export default function CloudCodePage() {
  ...
  const createSession = async (cli: CloudCodeCli, repo: string, authMode: CloudCodeAuthMode) => {
    const res = await fetch("/api/cloud-code/sessions", {
      method: "POST",
...
function AuthChip({ mode }: { mode?: CloudCodeAuthMode }) {
...
function NewSessionSheet({
  ...
  onClose: () => void;
  onCreate: (cli: CloudCodeCli, repo: string, authMode: CloudCodeAuthMode) => void;
  onManageAccount: () => void;
}) {
  const [cli, setCli] = useState<CloudCodeCli>("claude");
  const [authMode, setAuthMode] = useState<CloudCodeAuthMode>("bedrock");
  const [repo, setRepo] = useState("");
  const [connected, setConnected] = useState<{ claude?: boolean; codex?: boolean }>({});

  useEffect(() => {
    fetch("/api/cloud-code/auth")
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => {
        if (!d?.status) return;
...
function AccountSheet({ onClose, onToast }: { onClose: () => void; onToast: (m: string) => void }) {
  ...
  const [busy, setBusy] = useState(false);
  const [open, setOpen] = useState<CloudCodeCli | null>(null);
  const [secret, setSecret] = useState("");

  const load = useCallback(async () => {
    const res = await fetch("/api/cloud-code/auth");
    if (res.ok) setStatus((await res.json()).status || {});
  }, []);
  useEffect(() => { load(); }, [load]);

  const connect = async (cli: CloudCodeCli) => {
  ...
  const disconnect = async (cli: CloudCodeCli) => {
    setBusy(true);
    try {
      await fetch(`/api/cloud-code/auth?cli=${cli}`, { method: "DELETE" });
      await load();
    } finally {
      setBusy(false);
    }
  ...
  const rows: { cli: CloudCodeCli; name: string; how: string; cmd: string; placeholder: string }[] = [
    {
      cli: "claude",
      name: "Claude Pro / Max",
