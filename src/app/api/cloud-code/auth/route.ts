/**
 * GET    /api/cloud-code/auth          → which CLIs are connected to a plan
 * POST   /api/cloud-code/auth          → store a subscription credential
 *          { cli, token }                       (claude — from `claude setup-token`)
 *          { cli, authJson, label? }             (codex  — the ~/.codex/auth.json)
 * DELETE /api/cloud-code/auth?cli=claude|codex  → disconnect a CLI
 *
...
import { getAuthStatus, putCredential, deleteCredential, authConfigured } from "@/lib/cloud-code/auth-store";
import type { CloudCodeCli } from "@/lib/cloud-code/types";

export const dynamic = "force-dynamic";

function parseCli(v: unknown): CloudCodeCli | null {
...
export async function GET() {
  try {
    ...
    console.error("[cloud-code] auth status error:", err);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error("[cloud-code] auth set error:", err);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error("[cloud-code] auth delete error:", err);
    return NextResponse.json({ error: (err as Error).message }, { status: 500 });
  }
}
